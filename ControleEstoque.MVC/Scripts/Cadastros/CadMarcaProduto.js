...
function getTabela() {
    var table = $('#TabelaMarca').DataTable(
        {
    ...
    $("#TabelaMarca").on('click', 'a.btn-alterar', function () {
        var tr = $(this).closest('tr'),
            id = table.row(tr).data().MarcaProdutoId,
            url = '/MarcaProduto/Details',
            param = { 'id': id };
    ...
    $(document).on('click', '.btn-adicionar', function () {
        ...
        .on('click', '#btn_confirmar', function () {
            var param = get_dados_form(),
                url = param.MarcaProdutoId == 0 ? '/MarcaProduto/Create' : '/MarcaProduto/Edit';

            $.post(url, add_anti_forgery_token(param), function (f) {
                if (f.Resultado == "OK") {
                    if (param.MarcaProdutoId == 0) {
                        param.MarcaProdutoId = f.IdSalvo;
...
function get_dados_form() {
    return {
        MarcaProdutoId: $('#id_cadastro').val(),
...
function set_dados_form(dados) {
    $('#id_cadastro').val(dados.MarcaProdutoId);
