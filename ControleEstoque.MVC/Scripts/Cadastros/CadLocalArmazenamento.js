...
function getTabela() {
    var table = $('#TabelaLocalArmazenamento').DataTable(
        {
    ...
    $("#TabelaLocalArmazenamento").on('click', 'a.btn-alterar', function () {
        var tr = $(this).closest('tr'),
            id = table.row(tr).data().LocalArmazenamentoId,
            url = '/LocalArmazenamento/Details',
            param = { 'id': id };
    ...
    $(document).on('click', '.btn-adicionar', function () {
        ...
        .on('click', '#btn_confirmar', function () {
            var param = get_dados_form(),
                url = param.LocalArmazenamentoId == 0 ? '/LocalArmazenamento/Create' : '/LocalArmazenamento/Edit';

            $.post(url, add_anti_forgery_token(param), function (f) {
                if (f.Resultado == "OK") {
                    if (param.LocalArmazenamentoId == 0) {
                        param.LocalArmazenamentoId = f.IdSalvo;
...
function get_dados_form() {
    return {
        LocalArmazenamentoId: $('#id_cadastro').val(),
...
function set_dados_form(dados) {
    $('#id_cadastro').val(dados.LocalArmazenamentoId);
...
function abrir_form(dados) {
    ...
    bootbox.dialog({
        title: 'Cadastro de Locais de Armazenamento',
