...
function getTabela() {
    var table = $('#TabelaGrupoProduto').DataTable(
        {
    ...
    $("#TabelaGrupoProduto").on('click', 'a.btn-alterar', function () {
        var tr = $(this).closest('tr'),
            id = table.row(tr).data().GrupoProdutoId,
            url = '/GrupoProduto/Details',
            param = { 'id': id };
    ...
    $(document).on('click', '.btn-adicionar', function () {
        ...
        .on('click', '#btn_confirmar', function () {
            var param = get_dados_form(),
                url = param.GrupoProdutoId == 0 ? '/GrupoProduto/Create' : '/GrupoProduto/Edit';

            $.post(url, add_anti_forgery_token(param), function (f) {
                if (f.Resultado == "OK") {
                    if (param.GrupoProdutoId == 0) {
                        param.GrupoProdutoId = f.IdSalvo;
...
function set_dados_form(dados) {
    $('#id_cadastro').val(dados.GrupoProdutoId);
...
function get_dados_form() {
    return {
        GrupoProdutoId: $('#id_cadastro').val(),
...
function abrir_form(dados) {
    ...
    bootbox.dialog({
        title: 'Cadastro de Grupo Produto',
